...
namespace OpenXml
{
	...
	{
		// filePath 를 직접 명시하지 않고 파일을 
		// DB에 저장 해놓고 Stream으로 파일을 불러와서 사용 할 수도 있다.
		//byte[] byteIET = device.ietFile;
		//Stream filePath = new MemoryStream(device.ietFile);

		#region GooseSheet
		public  List<IET> ReadGooseSheet(String filePath)
		{
				...
				foreach (Row row in worksheetPart.Worksheet.Descendants<Row>().SkipWhile(r => r.RowIndex < dataStartRow.RowIndex).Skip(6))
				{
					...
					if (descriptionValue != null && dataTypeValue != null && pdTypeValue != null)
					{
						dataList.Add(new IET
						{
							...
							Pd = pdTypeValue,
							ld = ldTypeValue,
							ln_Prefix = lnPreFixTypeValue,
							ln = lnTypeValue,
							ln_Inst_No = lnInstNoTypeValue,
							fc = fcTypeValue,
							data_Object = dataObjectTypeValue,
							data_Attribute = dataAttributeTypeValue
						});
					}
		...
		public  List<IET_MMS> ReadMmsSheet(string filePath)
		{
			List<IET_MMS> dataList = new List<IET_MMS>();

			using (SpreadsheetDocument document = SpreadsheetDocument.Open(filePath, false))
			{
				WorkbookPart workbookPart = document.WorkbookPart;
				Sheet sheet = workbookPart.Workbook.Descendants<Sheet>()
					.Where(s => s.Name.Value.Contains("Data List"))
					.FirstOrDefault();
				...
				// colIndex[0] : 명칭 , [1] : Data Type, [2] : Signal Name
				int[] colIndex = new int[] { -1, -1, -1 };


				Row headRow = worksheetPart.Worksheet.Descendants<Row>().Skip(3).FirstOrDefault();

				if (headRow != null)
				{
					#region 5행에 있는 HeadData 명칭,DataType, SignalName 등
								 ...
								 .SkipWhile(r => r.RowIndex < dataStartRow.RowIndex).Skip(5))
					{
						...
						string tempDescriptionValue2 = GetCellValue(ReturnCell(row, colIndex[0] + 1), sharedStrings);
						string descriptionValue = tempDescriptionValue1 + " " + tempDescriptionValue2;
						string signalTypeValue = GetCellValue(ReturnCell(row, colIndex[1]), sharedStrings);
						string dataTypeValue = GetCellValue(ReturnCell(row, colIndex[2]), sharedStrings);

						#endregion

						if (descriptionValue != null && signalTypeValue != null && dataTypeValue != null)
						{
							dataList.Add(new IET_MMS
							{
								Description = descriptionValue,
								Address = signalTypeValue,
								DataType = dataTypeValue
							});
						}
		...
		private  Cell ReturnCell(Row row, int colIndex)
		{
